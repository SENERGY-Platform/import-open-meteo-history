...
OPEN_METEO_ARCHIVE_URL = OPEN_METEO_BASE_URL + "/v1/archive"

logger = get_logger(__name__)


def get_data(lat: float, long: float, start_date: str, end_date: str) -> Tuple[datetime, datetime, Optional[Units], List[Tuple[datetime, Value]]]:
    '''
    :param lat: Latitude
    :param long: Longitude
    :return: Units, Historical Weather Values
    '''
    lat = round(lat, 4)
    long = round(long, 4)

    url = OPEN_METEO_ARCHIVE_URL
    params = {'latitude': lat, 'longitude': long, 'timezone': 'auto',  'start_date': start_date, 'end_date': end_date,
